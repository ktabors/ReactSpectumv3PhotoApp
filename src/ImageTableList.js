...
const ImageTableList = (props) => {
  let [tableItems, setTableItems] = useState(props.items);
  ...
  let filteredItems = useMemo(() => list.items.filter(item => {
    ...
    } else if (imageTypeFilter.indexOf('png') !== -1 && item.filename.endsWith('.png')) {
      return contains(item.filename, filterText);
    }
      ...
      <View gridArea="content">
        <Table
          ...
          <TableHeader columns={columns}>
            {column => <Column width={column.width} allowsSorting={column.key === 'filename'}>{column.name}</Column>}
          </TableHeader>
              ...
              (<Row key={item.id}>
                {key => {
                  if (key === 'url') {
                    ...
                    return (
                      <Cell>
                        <ActionButton isQuiet onClick={() => filterImage(item['id'])}>
                          <ImageAutoMode />
                        </ActionButton>
                        <DialogTrigger>
